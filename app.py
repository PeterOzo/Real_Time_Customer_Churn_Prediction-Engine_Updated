...
from sklearn.impute import SimpleImputer  #  FIX 1: Added SimpleImputer
...
def preprocess_data(df):
    ...
    # FIX 2: Use assignment instead of deprecated inplace on chained operation
    ...
    df_processed['TenureSegment'] = pd.cut(
        df_processed['tenure'],
        bins=[0, 12, 24, 48, 100],
    ...
    # 3. Value-based features — guard against division by zero
    df_processed['ChargesPerService'] = (
        df_processed['MonthlyCharges'] / (df_processed['TotalServices'] + 1)
    )
    df_processed['TotalChargesPerTenure'] = (
    ...
    df_processed['AutoPay'] = (
        df_processed['PaymentMethod'].isin(['Bank transfer (automatic)', 'Credit card (automatic)'])
    ).astype(int)
    df_processed['DigitalPayment'] = (
        df_processed['PaymentMethod'].isin([
            'Electronic check', 'Credit card (automatic)', 'Bank transfer (automatic)'
        ])
    ).astype(int)
    
    # 5. Service adoption features
    df_processed['HasStreamingServices'] = (
        (df_processed['StreamingTV'] == 'Yes') | (df_processed['StreamingMovies'] == 'Yes')
    ).astype(int)
    df_processed['HasSecurityServices'] = (
        (df_processed['OnlineSecurity'] == 'Yes') |
        (df_processed['OnlineBackup'] == 'Yes') |
        (df_processed['DeviceProtection'] == 'Yes')
    ).astype(int)
    
    # 6. Contract value features
    # ✅ FIX 3: Added .fillna(1) to prevent NaN from unmatched contract values
    contract_mapping = {'Month-to-month': 1, 'One year': 12, 'Two year': 24}
    df_processed['ContractMonths'] = df_processed['Contract'].map(contract_mapping).fillna(1)
...
def prepare_features(df_processed):
    ...
    #  FIX 4: Replace inf values and fill any remaining NaN as safety net
...
def train_models(X, y):
    ...
    # FIX 5: Final NaN safety check before any model training
    ...
    try:
        ...
        sample_weight_nn = compute_sample_weight('balanced', y_train)
        nn_model.fit(X_train_scaled, y_train, sample_weight=sample_weight_nn)
...
def main():
    ...
    page = st.sidebar.selectbox(
        ...
        [" Executive Dashboard", " Data Intelligence", " ML Model Lab",
         " Prediction Engine", " Business Impact", " Advanced Insights"]
    )
    ...
    if page == " Executive Dashboard":
        ...
        <div class="insight-box">
            ...
            <p><strong>Mission:</strong> Develop advanced machine learning models to predict customer churn
            and enable proactive retention strategies for American Credit Acceptance (ACA).</p>
            ...
            <h4> Intelligent Class Balancing</h4>
            <ul>
        ...
        with col1:
            ...
            fig_contract = px.bar(
                ...
                color=churn_contract.values, color_continuous_scale='RdYlBu_r'
            )
    ...
    # ── DATA INTELLIGENCE ────────────────────────────────────────────────
        ...
        with feature_tabs[0]:
            ...
            with col1:
                fig_hist = px.histogram(
                    ...
                    opacity=0.7, color_discrete_sequence=['#667eea', '#764ba2'],
            ...
            with col2:
                fig_box = px.box(
                    ...
                    color='Churn', color_discrete_sequence=['#667eea', '#764ba2']
                )
        ...
        with feature_tabs[1]:
            ...
            churn_by_cat = df.groupby(selected_cat_feature)['Churn'].apply(
                lambda x: (x == 'Yes').mean()
            )
            fig_cat = px.bar(
                ...
                color=churn_by_cat.values, color_continuous_scale='RdYlBu_r'
            )
        ...
        if st.session_state.models_trained:
            ...
            with col2:
                ...
                fig_radar.update_layout(
                    title=" Multi-Metric Performance Radar", height=400,
            ...
            for i, (model_name, results) in enumerate(models_results.items()):
                ...
                fig_roc_curves.add_trace(go.Scatter(
                    ...
                    mode='lines', line=dict(color=colors[i % len(colors)], width=3)
                ))
            fig_roc_curves.add_trace(go.Scatter(
    ...
    # ── PREDICTION ENGINE ────────────────────────────────────────────────
        ...
        if not st.session_state.models_trained:
            ...
            <div class="warning-box">
                ...
                <p>Please train the models first in the <strong>ML Model Lab</strong> section.</p>
            </div>""", unsafe_allow_html=True)
            return
        ...
        if st.button(" Generate Churn Risk Assessment", type="primary"):
            ...
            base_prob = 0.25
            if contract == "Month-to-month":   base_prob += 0.25
            if payment_method == "Electronic check": base_prob += 0.15
            if tenure < 12:                    base_prob += 0.20
            if monthly_charges > 80:           base_prob += 0.10
            if internet_service == "Fiber optic": base_prob += 0.05
            
            models_results = st.session_state.models_results
            col1, col2, col3, col4 = st.columns(4)
            model_probs = []
            
            adjustments = {'Random Forest': 0.03, 'XGBoost': -0.02,
                           'Gradient Boosting': 0.01, 'Neural Network': -0.01}
            cols = [col1, col2, col3, col4]
            
            for i, (model_name, results) in enumerate(models_results.items()):
                adj = adjustments.get(model_name, 0)
                prob = min(base_prob + adj, 0.95)
            ...
            <div class="success-box">
                ...
                <p><strong>Risk Assessment:</strong> {
                    "High Risk - Immediate Action Required" if ensemble_prob > 0.7
                    else "Medium Risk - Monitor Closely" if ensemble_prob > 0.4
                    else "Low Risk - Continue Standard Service"
                }</p>
            </div>""", unsafe_allow_html=True)
    ...
    # ── BUSINESS IMPACT ──────────────────────────────────────────────────
        ...
        if not st.session_state.models_trained:
            ...
            <div class="warning-box">
                ...
                <p>Please train the models first in the <strong>ML Model Lab</strong>.</p>
            </div>""", unsafe_allow_html=True)
            return
        ...
        with col1:
            roi_df = pd.DataFrame(
                [(name, metrics['roi']) for name, metrics in business_metrics.items()],
                columns=['Model', 'ROI']
            )
            fig_roi = px.bar(
        ...
        with col2:
            value_df = pd.DataFrame(
                [(name, metrics['net_value']) for name, metrics in business_metrics.items()],
                columns=['Model', 'Net Value']
            )
            fig_value = px.bar(
                value_df, x='Model', y='Net Value',
                title=" Net Business Value by Model",
                color='Net Value', color_continuous_scale='Viridis'
            )
    ...
    # ── ADVANCED INSIGHTS ────────────────────────────────────────────────
        ...
        <div class="insight-box">
            ...
            <p><strong>Business Justification:</strong> Optimal balance of precision and recall.</p>
            <h5> Key Monitoring Metrics:</h5>
            <ul>
                <li><strong>Model Drift:</strong> Monitor feature distributions monthly</li>
                <li><strong>Performance:</strong> Track ROC AUC above {best_auc_model[1]['roc_auc'] - 0.05:.3f}</li>
